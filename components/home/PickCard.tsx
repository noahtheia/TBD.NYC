...
import TbdApprovedBadge from "@/components/ui/TbdApprovedBadge";
    ...
    >
      <div className="relative">
        <VenuePhoto
          ...
          sizes="288px"
        />
        <TbdApprovedBadge className="absolute left-3 top-3 shadow-sm" />
      </div>
      <div className="flex flex-1 flex-col p-4">
        <h3 className="font-display text-lg font-black leading-snug tracking-tight text-ink group-hover:text-ember">
          {pick.headline || v.name}
        </h3>
        {secondary && <p className="mt-0.5 text-sm font-medium text-zinc-500">{secondary}</p>}
        {pick.blurb && (
          <p className="mt-2 line-clamp-3 font-serif text-sm italic text-char">{pick.blurb}</p>
        )}
        <div className="mt-3 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-sm text-zinc-600">
          ...
          {v.happyHour === true && (
            <span className="rounded-full bg-glow/40 px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-ember">
              Happy hour
            </span>
          )}
        </div>
